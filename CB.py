    ...
    if process_query_button:
        if selected_query == "Choose your question":
            ...
            with st.chat_message("assistant", avatar="🤖"):
                # Placeholder for response time (shown just under the 🤖 icon)
                rt_placeholder = st.empty()
                message_placeholder = st.empty()
                full_response = ""

                # Start timing before OOD/generation
                ...
                # Stop timing after response is ready (before streaming animation)
                elapsed = time.perf_counter() - start_time
                rt_placeholder.markdown(f"<div class='response-time'>⏱️ Response time: {elapsed:.2f} s</div>", unsafe_allow_html=True)
